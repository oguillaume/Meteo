...
do{
	...
	try{
		...
		if ($socket === false) { 
			throw new Exception('socket create() a &eacute;chouée');
		} else { echo '<br>'; }
		...
		socket_close($socket);
		exit("Connexion à la station non r&eacute;alis&eacute;e");
	} finally{
	...
	{
		echo "connexion à la station en cours<br>";// test connexion
	...
	/*	if ($loop1[0]!=0x6 || $loop1[1]!='L' || $loop1[2]!='O' || $loop1[3]!='O'){
			echo 'probleme réception trame : $loop = / '.$loop1[0].'/'.$loop1[1].'/'.$loop1[2].'/'.$loop1[3];
		} else {*/
		...
		echo 'Temperature extérieure= '.$temp_out2.' °C <br>';
		...
		$date=date("d-m-Y H:i:s"); // recup date et heure de relevé
		echo $date." <br>";
